...
whitebox.py
═══════════
Implémentations des attaques WHITE-BOX : FGSM, PGD, C&W, pour les trois
modèles (MLP / LogReg / XGBoost), plus les helpers de gradient XGBoost et
l'affichage.

Ce fichier ne fait QUE définir des fonctions d'attaque. Il ne charge aucun
modèle, ne lit aucun fichier : les runners lui passent des wrappers déjà prêts.
...
# Seuil de décision exprimé côté logit : un sample est "évadé" si son logit
# passe sous ce seuil. Avec THRESHOLD = 0.5, THRESHOLD_LOGIT = 0.
...
# ══════════════════════════════════════════════════════════════
# GRADIENT XGBoost (approximation par différences finies)
# ══════════════════════════════════════════════════════════════

def _grad_xgb_vote(wrapper, X, eps_fd_list=EPS_FD_LIST):
    """
    Gradient XGBoost robuste : on calcule le gradient numérique à PLUSIEURS
    pas eps_fd, puis on prend la moyenne (amplitude) et un vote de signe
    (direction). Stabilise l'estimation d'une fonction en escalier bruitée.
    """
...
def _grad_xgb_smooth(wrapper, X, sigma=SMOOTH_SIGMA, K=SMOOTH_K, eps_fd=EPS_FD):
    """Variante SmoothGrad : moyenne du gradient sur K copies bruitées de X."""
    grads = []
    for _ in range(K):
...
def _xgb_bce_direction_sign(wrapper, X, y):
    ...
    grad_numerical donne ∂logit/∂x. Pour évader un sample y=1 il faut FAIRE
    BAISSER le logit. Le gradient de la BCE vaut ∇x BCE = (p − y)·∇x logit ;
    on monte la BCE avec +sign(∇x BCE). Cette fonction combine le vote
    multi-eps_fd (stabilité) et le facteur de direction (p − y).
    """
    ...
    if SMOOTH_GRAD:
        ...
        zero_mask = (sign_logit == 0)          # fallback si le vote donne 0
        if np.any(zero_mask):
...
def fgsm_xgb(wrapper, X_atk, y_atk, eps):
    """FGSM XGBoost : on monte la BCE (direction corrigée), pas le logit brut."""
...
def pgd_mlp(wrapper, X_atk, y_atk, eps,
            iters=PGD_ITERS, restarts=PGD_RESTARTS, alpha=None, verbose=True):
    """
    PGD multi-restart MLP.

    RNG : chaque restart tire son bruit via .uniform_() sur le générateur
    courant — le state avance naturellement entre restarts (inits différentes
    sans re-seed externe).
    best_logits initialisé à +inf → vraie compétition entre restarts.
    """
    ...
    for r in range(restarts):
        ...
        for _ in range(iters):
            ...
            nn.BCEWithLogitsLoss()(logits, y_t).backward()
            with torch.no_grad():
...
def pgd_logreg(wrapper, X_atk, y_atk, eps,
               iters=PGD_ITERS, restarts=PGD_RESTARTS, alpha=None, verbose=True):
    """PGD multi-restart LogReg. RNG numpy avance entre restarts, best_logits=+inf."""
    alpha = alpha or (eps / PGD_ALPHA_K)
    ...
    for r in range(restarts):
        ...
        x_adv = np.clip(X_atk + noise, X_atk - eps, X_atk + eps).astype(np.float32)
...
def pgd_xgb(wrapper, X_atk, y_atk, eps,
            iters=PGD_ITERS, restarts=PGD_RESTARTS, alpha=None, verbose=True):
    """
    PGD XGBoost. Direction BCE corrigée : +sign((p−y)·∂logit/∂x) → pour y=1,
    fait baisser le logit. RNG numpy avance entre restarts, best_logits=+inf.
    """
    alpha       = alpha or (eps / PGD_ALPHA_K)
    best_adv    = X_atk.copy().astype(np.float32)
    best_logits = np.full(len(X_atk), float("+inf"), dtype=np.float64)
...
def cw_logreg(wrapper, X_atk, y_atk, eps,
    ...
    """C&W LogReg — Adam manuel sur les logits, contrainte L∞ par clip."""
...
def cw_mlp(wrapper, X_atk, y_atk, eps,
    ...
    """
    C&W MLP — reparamétrage tanh + warm-start depuis pgd_mlp (corrigé).
    Le fix RNG de pgd_mlp se propage : le warm-start explore vraiment
...
def cw_xgb(wrapper, X_atk, y_atk, eps,
    ...
    """C&W XGBoost — Adam manuel + warm-start depuis pgd_xgb (corrigé)."""
    print("      [C&W XGBoost : warm-start PGD...]")
    X_adv = pgd_xgb(wrapper, X_atk, y_atk, eps, iters=20, restarts=3).astype(np.float64)
    print(f"      [C&W warm-start PGD terminé]")
        ...
        for model in ["MLP", "LogReg", "XGBoost"]:
            ...
            for _, row in sub.sort_values("attack").iterrows():
                ...
                print(f"  │    {row['attack']:<14} ASR {asr*100:5.1f}%  "
                      f"{_bar(asr, 15)}  F1 {row['f1_clean']:.3f}→{row['f1_adv']:.3f}  "
